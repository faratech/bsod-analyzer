...
| `STATS_SNAPSHOT_TTL_SECONDS` | In-process memo of the `/api/stats` snapshot (built from the Cloud Storage files) | Defaults `1800` |
| `STATS_BUCKET` | Cloud Storage bucket holding the JSON the BigQuery scheduled queries publish (live stats, baseline, corpus insights, AI priors) | Default `project-bigfoot-bsod-stats` |
| `CORPUS_ENABLED` | Stream every completed WinDBG job's full result into BigQuery `bsod_corpus.windbg_analyses` and acknowledge it to WinDbg-API (set `false` to disable) | Defaults on (needs `WINDBG_API_KEY`) |
| `CORPUS_BIGQUERY_DATASET` / `CORPUS_BIGQUERY_TABLE` | Corpus location | Default `bsod_corpus` / `windbg_analyses` |
...
`/api/stats` needs the Cloud Run metadata server (Cloud Storage) and answers 503
locally.
...
- **WinDBG corpus** (training data for the BSOD AI): `server/windbgCorpus.js` streams each completed WinDBG job, complete and unredacted, into the private dataset `bsod_corpus.windbg_analyses` (schema `bigquery/windbg_analyses.schema.json`; insertAll takes the JSON `result` column as a string, load jobs as an object). Only after BigQuery accepts the row is the job acknowledged via WinDbg-API `POST /api/v1/jobs/archived`; WinDbg-API prunes raw output of acknowledged jobs after 30 days, so a failed insert just leaves the job on the WinDBG server. Rows can repeat (retries, backfill overlap) — query `bsod_corpus.windbg_analyses_latest`. Every freshly generated AI report (web analyzer and external API) goes to `bsod_corpus.ai_reports` (schema `bigquery/ai_reports.schema.json`: exact prompt, raw response, parsed + final report, provider/model, token usage), joinable to the WinDBG row by `job_id` / `file_hash`; cache hits are not re-recorded. AI rows record the requested `model`, the provider-reported `model_version`, `service_tier` (e.g. `incentivized-tier` vs billed `default`) and the serving `route`. Historical jobs' AI reports (never stored originally) are regenerated by `scripts/regenerate-ai-reports.mjs` (`origin='regenerated'`, gpt-6-luna: OpenAI incentive first within a daily budget, then Experiential) using the site's own prompt builder, adapters and validator (`server/analysisReport.js`). History: `scripts/windbg-corpus-export.py` (on ST-WDBGAPI-01, Python 3.14+ once results are zstd-compressed) + `bq load`, then acknowledge the loaded ids
